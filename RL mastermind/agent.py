...
class Agent:
    ...
    def initialize_V(self):          #the idea is to make a dictionary and set value of all key(states)
                                     #as 0 (initializing)
        self.V = {}
        for idx in range(0, 6**4):
            self.V[env._number_from_index(idx)] = 0
    ...
    def restrict_possible_states(self, guess, feedback):
        new_states = [state for state in self.possible_states if env.score(guess, state)==feedback]
    ...
    def learn_from_move(self, action, feedback, reward):
        ...
        r = reward  
        self.make_move(action, feedback) 
        best_next_move, selected_next_move = self.learn_select_move()
        current_state_value = self.V[current_state] 
        best_move_value = self.V[best_next_move]  
